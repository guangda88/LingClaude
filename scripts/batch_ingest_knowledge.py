...
RAW_ENTRIES: list[tuple[str, str, str, str, str, str, str, str]] = [
    ...
    ("rule", "iron_rule", "推送权归各AI主理", "灵字辈推送权已回归各AI主理，审计流程待重构（原灵依独占已移交）", "security", "archive", "push,security,gate", "src/lingyi/_push_hook.py"),
    ("rule", "iron_rule", "不替用户做决策", "所有成员不得替用户做重要决策只能提供建议分析选项", "safety", "archive", "boundary,decision,safety", "docs/MISSION.md + docs/BOUNDARY_MANAGEMENT.md"),
    ("rule", "iron_rule", "推演必须标注", "推演必须标注为推演不确定必须说不确定不得将推演当事实", "safety", "archive", "hallucination,assertion,safety", "docs/MISSION.md + src/lingyi/constraint_layer.py"),
    ("rule", "high", "数据真实原则", "任何UI字段必须回答数据从哪来谁更新它；空值比假数字诚实", "architecture", "archive", "data,integrity,truth", "AGENTS.md"),
    ("rule", "high", "三层审计流程", "每次提交前必须通过L1单文件审计→L2交叉文件验证→L3同行复审REJECT阻止提交", "quality", "archive", "audit,quality,code-review", "docs/THREE_LAYER_AUDIT_PROCESS.md"),
    ("rule", "high", "依赖排序推送", "推送按依赖拓扑排序；某项目失败则中断整条推送链", "devops", "archive", "push,dependency,ordering", "src/lingyi/push_coordinator.py"),
    ("rule", "high", "没查过不许说", "LLM未调用任何工具验证事实性陈述时不得将其作为事实输出", "safety", "archive", "hallucination,prompt,safety", "docs/SELF_VERIFICATION_MECHANISM_20260410.md"),
    ("rule", "high", "方案讨论共识制", "方案讨论48小时无声明确反对即通过；反对必须附理由+替代方案", "governance", "archive", "council,consensus,governance", "docs/COUNCIL_RULES.md §2"),
    ("rule", "high", "全员决议全票制", "宪章修改/新成员加入/架构级变更须全票通过；弃权不等于反对", "governance", "archive", "council,governance,voting", "docs/COUNCIL_RULES.md §2"),
    ("rule", "high", "医疗查询关键词拦截", "ask.py 通过 _is_medical_query() 拦截含诊断/辨证/方剂等查询宁枉勿纵", "safety", "archive", "medical,guardrail,safety", "src/lingyi/ask.py:33-37"),
    ("rule", "high", "不硬编码密码密钥", "敏感数据用.env管理；.env/*.db/*.key不入Git；API key动态加载", "security", "archive", "security,credentials,git", "docs/DEVELOPMENT_PRINCIPLES.md"),
    ("pattern", "high", "两层架构模式", "每个领域两个文件：逻辑模块(纯业务逻辑)+命令模块(CLI封装)", "architecture", "archive", "architecture,pattern,cli", "AGENTS.md"),
    ("pattern", "high", "LLM模型自动降级", "按优先级尝试模型链；配额耗尽时缓存到下次重置时刻并自动降级", "resilience", "archive", "llm,fallback,resilience", "src/lingyi/llm_utils.py:172-204"),
    ("pattern", "high", "外部服务优雅降级", "每个外部调用包裹 try/except 服务不可用时返回 available=False 而非崩溃", "resilience", "archive", "resilience,graceful-degradation", "AGENTS.md"),
    ("pattern", "high", "Guard3自动回复链检测", "讨论末尾3条以上连续 auto_reply 标签则暂停唤醒成员防止 AI 空转", "safety", "archive", "council,guard,auto-reply", "src/lingyi/council.py"),
    ("pattern", "high", "推送日志审计追踪", "每次 push/commit 操作记录到 ~/.lingyi/push_logs/ 为 JSON 含时间戳/项目/结果", "devops", "archive", "push,logging,audit", "src/lingyi/push_coordinator.py:353-366"),
    ("lesson", "iron_rule", "2026-04-08未经审计推送事故", "18个commit未经审计推送到生产环境致9个CI失败；根因：审计流程缺失+安全文化缺失", "security", "archive", "incident,push,audit,lesson", "docs/SECURITY_INCIDENT_REPORT_20260408.md"),
    ("lesson", "iron_rule", "灵通+管道黑洞事件", "灵通+将LLM请求成功发送但未回传响应致全家族LLM调用瘫痪；教训：单点故障+不可审计", "reliability", "archive", "incident,pipeline,dark-code", "docs/INCIDENT_REPORT_LINGFLOW_PLUS_PIPELINE_20260409.md"),
    ("lesson", "high", "灵信幻觉事件教训", "29个灵信讨论中~20个为AI自导自演；需 source_type 标注和时间间隔异常检测", "hallucination", "archive", "incident,hallucination,identity", "docs/HALLUCINATION_RESEARCH.md"),
    ("lesson", "high", "自省靠不住用机制替代", "AI自省不可靠会忘记会偷懒；解决方案：用代码强制机制替代自觉", "safety", "archive", "lesson,self-improvement,mechanism", "docs/SELF_VERIFICATION_MECHANISM_20260410.md"),
    ("fact", "high", "核心价值观优先级", "守界(1)>惜时(2)>节约(3)>知己(4)>可靠(5)>诚实(6)；冲突时序号小的优先", "governance", "archive", "values,priority,mission", "docs/MISSION.md"),
    ("fact", "high", "灵依技术栈", "Python3.12+Click+FastAPI+SQLite+GLM+edge-tts+Whisper；本地文件存储", "architecture", "archive", "tech-stack,architecture", "README.md + AGENTS.md"),
    ("fact", "medium", "端口映射", "8000=灵知FastAPI；8080=智桥中继；8765=灵通+管道；WebUI已移交灵通+", "infrastructure", "archive", "ports,networking", "AGENTS.md"),
    ("skill", "high", "需求驱动开发", "只开发真正会用的功能；每写模块前问：我这周会用它吗？", "process", "archive", "development,process,prioritization", "docs/DEVELOPMENT_PRINCIPLES.md"),
    ("skill", "high", "最小可用渐进增强", "第一版只做一件事做到能用就上线；用起来发现不够再加", "process", "archive", "development,process,mvp", "docs/DEVELOPMENT_PRINCIPLES.md"),
    ("skill", "high", "代码简洁原则", "一个函数做一件事；嵌套不超过三层；逻辑代码单文件不超过300行", "quality", "archive", "code-quality,style,simplicity", "docs/DEVELOPMENT_PRINCIPLES.md"),
    ("rule", "medium", "丛林法则独立性", "每个项目独立生存不依赖其他项目也能运行；项目间按需互通", "governance", "archive", "ecosystem,independence,architecture", "docs/MISSION.md"),
    ("rule", "medium", "紧急通道", "安全漏洞/数据风险不受否决冷却限制；跳过讨论直接决策；事后补复盘", "governance", "archive", "council,emergency,governance", "docs/COUNCIL_RULES.md §6"),
    ("rule", "medium", "不做清单", "不搞微服务/过度抽象/100%测试覆盖/花哨UI/提前优化/越界行为", "process", "archive", "development,anti-pattern,process", "docs/DEVELOPMENT_PRINCIPLES.md"),
    ...
    # --- Cross-ecosystem ---
    ("fact", "info", "灵字辈生态总览", "灵通/灵克/灵信/灵扬/灵极优/灵犀/灵知/智桥/灵研/灵通+/灵通问道/灵网 12个核心项目", "ecosystem", "ecosystem", "ecosystem,ling-family", "灵字辈仓库描述.txt"),
    ...
    ("fact", "info", "灵字辈部署阶段", "阶段1基础设施(灵信/灵犀/智桥)→阶段2服务(灵知/灵研)→阶段3应用(灵极优/灵通/灵通+)→阶段4辅助(灵克/灵扬)", "operations", "ecosystem", "deployment,phases", "ling-background-services-summary.md"),
]
