...
1. **不替代Crush** — Crush是执行单元，灵OS是调度器。不同粒度的出入+流转，不是竞品
2. **不重复systemd** — systemd管service做对了。灵OS管systemd不管的（token/显存/task/context）
3. **需要什么管什么** — 先跑起来最简内核，碰到边界再拆墙。systemd现在不拆，以后需要再拆
