...
为什么是插片：**插片和主干的区别不在"做什么"，在"能不能换位置"。一固定位置，功能就被锁死了。**

灰区判断可以放在主干流的任意位置：

```
前置（query之前）: 信息还没进来，先判断该不该接收 → 成本最低（0损害）
中间（query之后，transition之前）: 信息进来了，判断该不该流转 → 中等（可回滚）
后置（transition之后）: 状态已经变了，判断结果对不对 → 成本最高（要修复）
```
...
如果把灰区锁死在transition一个位置（"内置属性"），灵犀的前置校验和pytest的后置验证就解释不了。灰区是插片，因为它可以放在任意位置。
