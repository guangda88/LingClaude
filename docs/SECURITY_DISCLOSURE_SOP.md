...
- 内部通知：LingBus `security-disclosure` 频道 + 灵族生态群 + 灵研 + 灵律
...
| **责任 Agent** | 灵克（代码 / proxy）、灵研（数据 / 论文）、灵律（合规）、灵通（运维 / lingflow） |
...
| 严重性 | 目标修复时效 | 验证要求 |
|--------|------------|---------|
| **Critical**（RCE / 大规模数据泄露） | T+30d | 全量回归 + 灵研 + 灵通 双签 |
...
| 法务审查 | 灵律 (linglaw) | 高风险披露必介入 |
| 运维配合 | 灵通 (lingflow) | 灰度 / 回滚 / 监控 |
