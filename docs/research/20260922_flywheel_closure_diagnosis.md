...
| **v4.3** | 灵克 | **F5 双条件满足并落地（2026-09-23）**：① corrections 1→1195（`scripts/corrections_backfill.py`：hard_interrupt 1194 session → 关联末次 tool_error 回填，source=system_backfill conf=0.6，幂等）+ turn_learner 已有 user-correction 流持续；② verify_ledger 落账自动化（`audit_watch._run_f5_hooks` 入 sweep 层：回填+`rule_conf_governor.py` conf 三档治理 → 脚本输出作 evidence 落 verify_ledger，实测 verified=True）。**conf 治理三档**：evidenced 0.9（关键词与 verify claim 交叉≥2）/ neutral 0.6 / corrected 0.5（错误锚点片段命中 correction 语料），SET-to-cap 语义（治理器为机器生成规则 conf 权威，幂等自愈——踩坑记录：min-clamp 首版 token 碰撞误压 61 条且无法恢复，已改 SET 后重跑归位）。终态：2 evidenced / 64 neutral / 5 corrected。测试 11+25+18 绿；F5 条件③（F0 基线≥1周）继续按墙钟积累 |
