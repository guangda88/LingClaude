"""灵知（lingzhi）插片——家族批量插片化（批3）。

载体定性（2026-09-18 实测）：服务型（/home/ai/lingzhi）
...
账本对账：org_member/lingzhi.json 的 trust/plug 声明与 manifest 一致（测试锚定）。
"""
...
class LingZhiAgentPlugin(McpAgentPluginBase):
    """agent/lingzhi 灵知插片（service 型，manifest 全驱动）。"""
...
def register(registry) -> None:
    ...
    registry.register(SeamType.AGENT, LingZhiAgentPlugin.name, LingZhiAgentPlugin())
