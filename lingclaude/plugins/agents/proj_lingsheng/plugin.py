"""灵声（lingsheng）插片——家族批量插片化（批4）。

载体定性（2026-09-18 实测）：载体缺席（纯登记态）（无本地载体）
...
账本对账：org_member/lingsheng.json 的 trust/plug 声明与 manifest 一致（测试锚定）。
"""
...
class ProjLingshengPlugin(McpAgentPluginBase):
    """proj/lingsheng 灵声插片（absent 型，manifest 全驱动）。"""
...
def register(registry) -> None:
    ...
    registry.register(SeamType.AGENT, ProjLingshengPlugin.name, ProjLingshengPlugin())
