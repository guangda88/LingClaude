"""灵网（lingweb）插片——家族批量插片化（批3）。

载体定性（2026-09-18 实测）：服务型（/home/ai/lingweb）
...
账本对账：org_member/lingweb.json 的 trust/plug 声明与 manifest 一致（测试锚定）。
"""
...
class LingWebAgentPlugin(McpAgentPluginBase):
    """agent/lingweb 灵网插片（service 型，manifest 全驱动）。"""
...
def register(registry) -> None:
    ...
    registry.register(SeamType.AGENT, LingWebAgentPlugin.name, LingWebAgentPlugin())
