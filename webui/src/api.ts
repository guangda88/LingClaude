...
export async function getModels(): Promise<ModelInfo[]> {
  // lingclaude-webui: 端点从 AtomCode 的 /models 适配为骨架的 /status
  const r = await fetch('/status', { headers: authHeaders() });
  if (!r.ok) throw new Error(`list models failed: ${r.status}`);
  const body: unknown = await r.json();
  const models = (body as { models?: ModelInfo[] })?.models;
  if (!Array.isArray(models)) throw new Error('list models returned an invalid payload');
  return models;
}
