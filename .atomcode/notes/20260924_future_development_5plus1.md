# 灵族未来发展·5+1 层架构修订规划 v0.3

> **状态**：草案 v0.3（v0.2 + codex 战略层评议吸收；修订处以 `[v0.3修订]` 标注）
> **作者**：灵克会话（基于 atomcode 授权）
> **生成日期**：2026-09-24
> **修订历程**：用户原话 4 层 → 诊断为 5+1 层 → 派 5 家 agent 评议（ac/cc/crush/opencode 实返，codex 超时）→ v0.1 整合 → **v0.2（同日）：灵克逐项核对仓内现状，修正 3 处过时/失准断言 + 3 处结构性修订** → **v0.3（同日）：codex 重派评议返回（判定"需修订后再裁"），吸收其发现：N6 撞号改 N9、§1.4/§1.5/§九.2 降级同步、"铁律 7 候选"标签修正、循环除名质疑补注**

---

## 〇、一句话总览

**5+1 层架构 = `L0 治理宪章（gov/ 域，横切）` + `L1 core/` + `L2 agent/` + `L3 os/` + `L4 cap/`（拆 L4a/L4b）+ `L5 hw/`**；临界路径是 **L1 → L2**（`coding.py` 装配外移 + `agent-gateway` 熔断与 manifest 化），与铁律 7（已升格，`LINGYUAN_IRON_LAW.md` :117，2026-09-17 用户裁决）的 5 域前缀同构，**不另起栈**。三个月内必须落地的硬约束是 **token schema 收尾**（`[v0.2修订]` `session_token_sink` 已于 2026-09-23 落地（commit a66e04c，StateStore 路径已见真实数据）；剩余收尾 = session_history.json 老路径接入（仍 0 含字段，须避免双轨台账）+ cost 换算字段）+ **rss_watchdog 挂 gov/ 命名空间**（`[v0.2修订]` 已是 RSS 看门狗守卫（ops/rss_watchdog.py，500MB/1000MB 阈值+基线重置），`[v0.3修订]` 修正：代码自称"N6"与铁律注册表撞号（N6=口径成本互证，:392；N1-N7 全占，N8 留给环境守卫）——本规划改挂 **N9**，代码侧改名挂 arch_audit_task 待办；待办是从 ops 域挂 gov/ 前缀 + 补启动 free-RAM 门，防 8 月 21GB thrash 复蹈）+ **5+1 ↔ ROADMAP.md 映射回写**（否则规划与既有路线图互相不可见）。
...
- **新增 L0 专用前缀 `gov/`**（治理/宪章/守卫，落 seam 时用 `gov/iron_law_N`、`gov/guard/N1` 等专用命名空间；不接受无前缀的 L0 插片）
- 横切层（测试/可观测/商业化/环境约束）走 `gov/xcut/<name>` 命名，与 L0 合并表达
...
| **新增交付（本规划建议）** | ① `gov/iron_law_N` 命名空间前缀规则段写回铁律第 117 行；② 候选铁律 5/6/7/8 升格裁决（9-17 已升格，候选区保留作档案）；③ J6 用户价值判据（建议加入）；④ N8 环境约束声明守卫（crush 提案） |
...
| 层 | 里程碑 | 验收 |
|---|---|---|
| **L0** | ① 5+1 层宏图附录写回 `LINGYUAN_IRON_LAW.md`（gov/ 命名规则段）；② J6 用户价值判据草案；③ N8 环境约束声明守卫草案 | 文档可查 |
...
| **L0** | rss_watchdog 挂 gov/ 守卫 `[v0.3修订]`（守卫已实存；编号修正：代码自称 N6 撞号，改挂 **N9**，代码侧改名挂 arch_audit_task；剩：ops→gov/guard/N9 前缀迁移 + 启动 free-RAM 门）| 8 月 21GB thrash 复蹈路径阻断 |
...
| **L3** | 1 台异 OS 设备最小验证（1 条指令 + 1 次回执）| 实跑 PASS |
| **L4a** | cap_browser 端到端跑通 1 个真实场景（如下单或政务查询）| T3 契约漂移 record 落盘 |
| **L4** | 统一应用接入协议草案 v0.1 | 草案可查 |
| **L5** | 5 家算力供应商建台账（价格/配额/故障域）| 台账可查 |
| **L5** | GPU 评估 v4 重做（含 SLA/账单/合同）| 取代 v3 |
| **ROADMAP** | ROADMAP.md 加附录"P0-P3 落入 5+1 层映射表" | 文档可查 |
...
| 位置 | 建议 |
|---|---|
| `:117`（铁律 7 域前缀） | 加 gov/ 域前缀段落，明文规定 L0 不在 5 业务域内 |
...
| 风险 | 概率 | 影响 | 降级路径 |
|---|---|---|---|
| L1 装配外移期间自优化线又启并行 | 中 | 高（lingxi 5-failed 复蹈）| work_claim 强锁 + 任何新功能冻结到 M1 落地 |
...
- **.github/、CI/CD、容器化**：如有 k8s/terraform/argo 是一整层
- **scripts/ 全集未读**：除 self_audit_trigger.py + atomcode-cgroup/shim 外，可能漏运维脚本
- **benchmarks/ 目录未查**：性能基准数据可能影响 L5 算力账单
...
1. **本周**：主人裁决本规划 v0.3（决定是否作为未来发展基线；裁决前请先过目 `[v0.2修订]`/`[v0.3修订]` 标注处——v0.1 三处事实断言已过时，v0.3 已吸收 codex 评议）
2. **本周**：ROADMAP.md 加 5+1 映射附录（最小成本、与既有路线同步）
3. **本周**：LINGYUAN_IRON_LAW.md 加 gov/ 前缀规则段
4. **下周**：token schema 收尾立项 `[v0.2修订]`（拆 arch_audit_task；sink 已落地，立项范围为老路径接入 + cost 字段 + 灵忆镜像消费）
5. **下周**：rss_watchdog 挂 gov/ 守卫立项 `[v0.3修订]`（守卫已实存；编号修正：代码自称 N6 撞铁律注册表 N6=口径成本互证，改挂 **N9**；范围 = ① rss_watchdog.py 自称改 N9（他成员代码，走 arch_audit_task）+ ops→gov/guard/N9 前缀迁移 + 启动 free-RAM 门）
6. **下周**：L1 装配外移 M1 立单（待自优化线收口）
7. ~~2 周内：重派 codex 补战略层评议~~ `[v0.3修订]` **已完成**（同日重派成功，产物 `.atomcode/notes/20260924_codex_review_5plus1_v0.2.md`，发现已吸收进本版）
8. **1 月内**：L2 gateway 熔断 + 5 家 manifest 化立项
...
— v0.3 草案，2026-09-24，待主裁与回写（v0.1→v0.2 灵克实测修订、v0.2→v0.3 codex 评议吸收，均以行内标注保留痕迹；codex 评议原文见 `.atomcode/notes/20260924_codex_review_5plus1_v0.2.md`）